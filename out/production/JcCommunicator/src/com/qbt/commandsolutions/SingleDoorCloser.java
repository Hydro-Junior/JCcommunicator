...
public class SingleDoorCloser extends CommandHandler {

	public SingleDoorCloser(Command command) {
		super(command);
	}
	public SingleDoorCloser(Command command,int timeOut) {
		super(command,timeOut);
	}

	@Override
	public void run() {
		Logger logger = Logger.getLogger(SingleDoorCloser.class);
		switch(this.command.getType()){
		case 0:
			DBHandler.updateCmd_TP(command, 3);
			Request req = new Request("METER_CLOSEDOOR",new DataOfReq(command.getContentValue1(),command.getContentValue3()));
			...
			try {
				...
				if(job.getJSONObject("error").getString("code").equals("0")){
					...
					DBHandler.updateStrobeState(2, centerId, command.getContentValue3());
