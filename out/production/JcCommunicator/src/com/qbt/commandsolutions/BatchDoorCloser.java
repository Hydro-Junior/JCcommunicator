...
public class BatchDoorCloser extends CommandHandler {

	public BatchDoorCloser(Command command) {
		super(command);
	}
	public BatchDoorCloser(Command command , int timeOut) {
	...
	@Override
	public void run() {
		Logger logger = Logger.getLogger(BatchDoorCloser.class);
		switch(this.command.getType()){
		case 0:
			DBHandler.updateCmd_TP(command, 3);
			Request req = new Request("METER_BATCHCLOSEDOOR",new DataOfReq(command.getContentValue1()));
			String s = GsonUtil.objToJson(req);
			try {
				String ss = (String)ReturnGetter.getReturn(s);
				JSONObject job = new JSONObject(ss);
				//如果返回值没有错，设置阀门状态为关，完成命令后更改命令状态。
				if(job.getJSONObject("error").getString("code").equals("0")){
					...
					DBHandler.batchSetSrobeState(2, centerId);
		...
		case 1:
			...
			Request req2 = new Request("METER_BATCHCLOSEDOOR",new DataOfReq(command.getContentValue1()));
